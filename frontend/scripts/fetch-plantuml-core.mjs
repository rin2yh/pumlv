...
// The upstream TeaVM build refuses to render diagrams whose layout exceeds
// 4096px on either axis, which breaks large-but-valid sequence/ER diagrams.
// We patch the two dimension checks so the limit is effectively removed.
const PLANTUML_PATCHES = [
  { from: "p<=4096.0", to: "p<=2147483647.0" },
  { from: "q<=4096.0", to: "q<=2147483647.0" },
];
...
function patchPlantumlJs(filePath) {
  ...
  if (patched) {
    ...
    console.log("plantuml.js: removed 4096px dimension limit");
  }
}
